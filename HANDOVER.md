...
| | GAIN | GND (100kΩ) | ★ 6dB 고정 (8Ω 5W 스피커 교체 후 변경, 3dB→6dB) |
...
| 2026-05 | 스피커 4Ω 3W → 8Ω 5W×2 병렬 | 물리적 클리핑 해소, 음질 개선 |
| 2026-05 | GAIN 핀 100Ω → 100kΩ (3dB→6dB) | 아날로그 게인 증가, SNR 개선 |
| 2026-05 | ffmpeg GAIN_DB 6→3, LIMIT 0.85→0.90 | 하드웨어 게인 보상, 리미터 여유 확대 |
| 2026-05 | maxVolume 기본값 11→16 | 5W 스피커 여유 반영, 볼륨 범위 확대 |

**스피커**: 8Ω 5W 듀얼 스피커 (40mm 자기 캐비티) × 2개 **병렬 배선** → 합성 4Ω.  
각 스피커 최대 수신 전력 1.6W (정격 5W 대비 32%) — 물리적 클리핑 해소.

#### 듀얼 스피커 병렬 배선

```
MAX98357A OUT+  ─┬─ Speaker1(+)    Speaker1(−) ─┬─ MAX98357A OUT−
                 └─ Speaker2(+)    Speaker2(−) ─┘
```
> ⚠️ 직렬(16Ω) 연결 금지 — 출력 0.5W로 급감하여 저음 손실 심각.
...
> **설계 이유**: 시계 모드 → 라디오 ON은 RIGHT SHORT만. LONG은 라디오 모드에서 시계 전환 전용.  
> AP 진입은 양쪽 동시 10s만 (싱글 버튼 10s는 AP 모드 진입 안 함 — 코드 확인됨).
